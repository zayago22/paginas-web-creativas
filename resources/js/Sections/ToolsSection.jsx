...
function ToolCardMobile({ tool, onTap }) {
    return (
        <button
            type="button"
            onClick={onTap}
            className="w-full text-left p-4 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl flex items-center gap-3 active:scale-[.98] transition-transform relative overflow-hidden"
        >
            {/* Desktop badge */}
            ...
            <div className="min-w-0">
                ...
                <p className="text-xs text-[var(--text-muted)] leading-relaxed mt-0.5 line-clamp-1">{tool.short_description}</p>
            </div>
        </button>
    );
}
    ...
    return (
        <div
            ...
            className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[2000] flex items-center gap-3
                ...
                bg-[var(--bg-card)] backdrop-blur-md border border-[var(--border-color)] shadow-[0_8px_32px_var(--shadow-color)]
        ...
        <section className="py-28" id="herramientas">
            <div className="max-w-[1200px] mx-auto px-6 text-center">
                <Reveal>
                    ...
                    <p className="text-[var(--text-secondary)] text-lg max-w-[600px] mx-auto leading-relaxed">Optimiza tus imágenes, genera QR y más. Sin registro, sin subir archivos a servidores.</p>
                </Reveal>
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-16">
                        {tools.map((tool, i) => (
                            <Reveal key={tool.id} delay={i * 80}>
                                {tool.url ? (
                                    <Link href={tool.url} className="group block p-7 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl hover:border-[#0090ff]/20 hover:-translate-y-1 hover:shadow-[0_16px_50px_var(--shadow-color)] transition-all text-center relative overflow-hidden">
                                        ...
                                        <p className="text-xs text-[var(--text-muted)] leading-relaxed">{tool.short_description}</p>
                                    </Link>
                                ) : (
                                    <div className="p-7 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl opacity-60 text-center relative">
