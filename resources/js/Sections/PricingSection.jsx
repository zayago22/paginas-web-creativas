                    ...
                    {services.map((svc, i) => (
                        <Reveal key={svc.id} delay={i * 120}>
                            <div className={`text-left p-10 rounded-2xl relative transition-all hover:-translate-y-1
                                ${svc.featured
                                    ? 'bg-gradient-to-b from-[#0090ff]/8 to-[var(--bg-card)] border border-[#0090ff]/30'
                                ...
                                <div className="text-sm font-semibold text-[var(--text-secondary)] mb-2">{svc.name}</div>
                                <div className="text-4xl font-black mb-1">${Number(svc.price).toLocaleString()} <span className="text-sm font-normal text-[var(--text-secondary)]">{svc.price_label}</span></div>
                                <div className="text-sm text-[var(--text-muted)] mb-6">{svc.price_period}</div>
                                <p className="text-sm text-[var(--text-secondary)] leading-relaxed mb-6 pb-6 border-b border-[var(--border-color)]">{svc.short_description || svc.description}</p>
                                ...
                                <a href={`${whatsappBase}?text=${encodeURIComponent(`Hola! Me interesa el plan ${svc.name} ($${Number(svc.price).toLocaleString()})`)}`}
                                   ...
                                   className={`block w-full text-center py-3.5 rounded-lg font-semibold text-sm transition-all
                                       ${svc.featured
                                           ...
                                           : 'border border-[var(--border-color)] text-[var(--text-primary)] hover:border-[#0090ff] hover:bg-[#0090ff]/5'}`}>
