        ...
        <div className="space-y-3">
            ...
            {files.map((f, i) => (
                <div
                    ...
                    style={{ background: 'var(--briefing-surface)', border: '1px solid var(--briefing-border)' }}
                >
                    <File size={18} style={{ color: '#6C63FF', flexShrink: 0 }} />
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate" style={{ color: 'var(--briefing-text)' }}>{f.name}</p>
                        {f.size && <p className="text-xs" style={{ color: 'var(--briefing-text-muted)' }}>{formatFileSize(f.size)}</p>}
                    </div>
            ...
            {files.length < MAX_FILES && (
                <div
                    ...
                    style={{ border: '1.5px dashed var(--briefing-border)', background: 'var(--briefing-surface)' }}
                    onClick={() => inputRef.current?.click()}
                >
                    <Upload size={28} style={{ color: 'var(--briefing-text-muted)' }} />
                    <p className="text-sm text-center" style={{ color: 'var(--briefing-text-muted)' }}>
